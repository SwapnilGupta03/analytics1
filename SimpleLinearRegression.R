...
df1 = data.frame(X,Y) #create data frame
head(df1)
...
abline(lm(df$Y ~ df$X), lty=3, lwd=4, col='green') # with regression line lty= linetype
...
fit1
summary(fit1)
# find p value, if p value<0.5 then linear model exists and x is significant 
...
#or
(R = df$Y-fitted(fit1)) #will also calculate residual
...
#linear regression in short can also be written as
df
fit = lm(Y ~ X, data=df)
summary(fit)
plot(fit)
